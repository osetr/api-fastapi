...
@app.post("/users/new_user", response_model=schemas.UserCreate, status_code=status.HTTP_201_CREATED)
...
def read_posts(user_login: str, db: Session = Depends(get_db), current_user = Depends(crud.get_current_user)):
    if user_login != current_user.current_user:
        raise HTTPException(
            ...
            detail="There're no permissions for user " + current_user.current_user
        )
